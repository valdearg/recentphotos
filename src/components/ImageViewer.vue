	...
	<div v-if="mounted" class="viewer-overlay" @click="onOverlayClick" @mousemove="onActivity"
		...
		<div ref="toolbar" class="viewer-toolbar" :class="{ 'viewer-ui-hidden': uiHidden }">
			...
			<div class="viewer-actions">
				...
				<button v-if="currentImage?.gelbooruUrl" class="external-source-button" @click="openGelbooru"
					title="Open Gelbooru post" aria-label="Open Gelbooru post">
					G
				</button>
...
export default {
	...
	methods: {
		...
		openGelbooru() {
			const url = this.currentImage?.gelbooruUrl
			if (url) window.open(url, '_blank', 'noopener')
		},
