	...
	>
		<div ref="toolbar" class="viewer-toolbar" :class="{ 'viewer-ui-hidden': uiHidden }">
			...
			<div class="viewer-actions">
				...
				<button @click="resetView" title="Reset" :disabled="isVideo">
					<svg viewBox="0 0 24 24"><path d="M12 5v4l3-3-3-3v4a7 7 0 1 0 7 7"/></svg>
				</button>
				...
				<button @click="toggleSlideshow" title="Slideshow">
					<svg v-if="!slideshowRunning" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
					<svg v-else viewBox="0 0 24 24"><path d="M6 5h4v14H6zm8 0h4v14h-4z"/></svg>
				</button>
				<button @click="openInNewTab" title="Open in new tab">
					<svg viewBox="0 0 24 24"><path d="M14 3h7v7M21 3l-9 9M5 5h6v2H7v10h10v-4h2v6H5z"/></svg>
				</button>
		...
		<div ref="body" class="viewer-body" :class="{ 'viewer-ui-hidden': uiHidden }">
			<div class="viewer-side viewer-side-left" @click.stop="previousImage">
				<button type="button" class="nav-button" :disabled="!hasPrevious" title="Previous" @click.stop="previousImage">
			...
			<div
				...
				@mouseup="stopDrag"
				@mouseleave="stopDrag"
				@dblclick.stop="toggleDoubleClickZoom"
...
<style scoped>
.viewer-overlay { position: fixed; inset: 0; z-index: 100000; background: rgba(0, 0, 0, 0.95); display: flex; flex-direction: column; cursor: default; }
.viewer-toolbar { display: flex; justify-content: space-between; align-items: center; gap: 16px; padding: 12px 16px; color: white; background: rgba(0, 0, 0, 0.4); transition: opacity 0.2s ease; }
.viewer-title { font-size: 16px; font-weight: 600; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.viewer-actions { display: flex; gap: 8px; flex-wrap: wrap; justify-content: flex-end; align-items: center; }
.viewer-actions button { appearance: none; -webkit-appearance: none; display: inline-flex; align-items: center; justify-content: center; width: 38px; height: 38px; padding: 0; margin: 0; border: 1px solid rgba(255,255,255,0.25); background: rgba(255,255,255,0.08); color: white; border-radius: 8px; cursor: pointer; font: inherit; line-height: 1; box-sizing: border-box; }
.viewer-actions button:hover { background: rgba(255,255,255,0.18); }
.viewer-actions button:disabled { opacity: 0.35; cursor: default; }
.viewer-actions svg { width: 18px; height: 18px; stroke: white; stroke-width: 2; fill: none; }
.viewer-body { flex: 1; display: flex; align-items: center; padding: 16px; gap: 16px; min-height: 0; transition: opacity 0.2s ease; }
.viewer-side { width: 90px; height: 100%; display: flex; align-items: center; justify-content: center; flex: 0 0 90px; }
.viewer-side-left { justify-content: flex-start; }
.viewer-side-right { justify-content: flex-end; }
.viewer-image-wrap { flex: 1; display: flex; justify-content: center; align-items: center; overflow: hidden; user-select: none; min-width: 0; min-height: 0; touch-action: pan-y; }
.viewer-image { max-width: 100%; max-height: calc(100vh - 180px); object-fit: contain; will-change: transform; transition: transform 0.08s linear; }
.viewer-video { max-width: 100%; max-height: calc(100vh - 180px); border-radius: 12px; background: black; }
.nav-button { width: 60px; height: 60px; display: flex; align-items: center; justify-content: center; background: rgba(255,255,255,0.1); color: white; border: none; border-radius: 999px; cursor: pointer; flex: 0 0 auto; transition: background 0.2s ease, box-shadow 0.2s ease, opacity 0.2s ease; }
.nav-button:hover:not(:disabled) { background: rgba(255,255,255,0.2); box-shadow: 0 0 18px rgba(255,255,255,0.18); }
.nav-button:disabled { opacity: 0.35; cursor: default; }
.nav-button svg { width: 28px; height: 28px; stroke: white; stroke-width: 2; fill: none; }
.viewer-info { position: absolute; top: 76px; right: 20px; background: rgba(0,0,0,0.7); padding: 12px; color: white; border-radius: 10px; max-width: min(420px, calc(100vw - 40px)); line-height: 1.5; word-break: break-word; transition: opacity 0.2s ease; }
.viewer-footer { text-align: center; color: white; padding: 10px; transition: opacity 0.2s ease; }
.viewer-ui-hidden { opacity: 0; pointer-events: none; }
</style>
